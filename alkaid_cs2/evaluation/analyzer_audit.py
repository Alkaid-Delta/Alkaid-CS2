...
ALLOWED_AUDIT_RESULTS = frozenset({"blocked", "planned", "completed", "failed"})

# 已知固定錯誤碼（Phase 6.4C2-B0 / B0.1）
...
def validate_audit_manifest(audit: dict) -> list[str]:
    """嚴格 audit schema 驗證（Phase 6.4C2-B0.1）；回傳錯誤清單。"""
    errors: list[str] = []
    unknown = set(audit) - ALLOWED_AUDIT_FIELDS
    if unknown:
    ...
    for count_field in ("eligible_case_count", "eligible_image_count",
